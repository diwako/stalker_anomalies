...
if(hasInterface) then {
    ...
    private _source = _trg getVariable ["anomaly_particle_source", objNull];
    ...
    if(!isNull _source) then {
        ...
        if(_plr distance _trg < ANOMALY_IDLE_DISTANCE) then {
            ...
            [_source, "idle"] call anomalyEffect_fnc_electra;
            _trg setVariable ["anomaly_particle_source", _source];
        };
    };
};