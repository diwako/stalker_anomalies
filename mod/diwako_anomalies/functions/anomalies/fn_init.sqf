        ...
        if!(isNil "_type") then {
            if((_pos distance _x) <= ANOMALY_IDLE_DISTANCE) then {
                ...
                if(isNull _source) then {
                    // create idle effect
                    // private _proxy = "building" createVehicleLocal position _x;
                    ...
                    _source setPosWorld (getPosWorld _x);
                    _source setVariable ["anomaly_trigger", _x];
                    // _source enableSimulation false;
                    private _arr = [_source, "idle"];
                    switch (_type) do {
                        ...
                        case 5: {
                            ["Create fog anomaly", [
                                ...
                                ],
                                {
                                    ...
                                    _args params ["_pos"];
                                    [_pos,_radius,_rectangle] remoteExec ["anomaly_fnc_createFog",2]
                                }, {}, [_pos]
                            ] call zen_dialog_fnc_create;
                        };
                        case 6: { [_pos] remoteExec ["anomaly_fnc_createFruitPunch",2] };
