...
_source setPosWorld (getPosWorld _trg);
if(hasInterface) then {
    [_source, "active"] call anomalyEffect_fnc_springboard;
};
        ...
        if(alive _x) then {
            ...
            if(!(isPlayer _x)) then {
                ...
                if(!isNil "ace_medical_fnc_addDamageToUnit") then {
                    ...
                    _dam = (missionNamespace getVariable ["ace_medical_playerDamageThreshold", 1]) * 1.5;
                    [_x, _dam, selectRandom ["head", "body", "hand_l", "hand_r", "leg_l", "leg_r"], "stab", _x] remoteExec ["ace_medical_fnc_addDamageToUnit", _x];
                } else {
