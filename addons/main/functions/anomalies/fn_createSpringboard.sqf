...
[QGVAR(setTrigger), [
    ...
    ["ANY", "PRESENT", true], // activation
    [format ["this && !(thisTrigger getVariable ['%1',false]) && {([] call CBA_fnc_currentUnit) in thisList}", QGVAR(cooldown)], format ["[thisTrigger] spawn {params['_thisTrigger']; _proxy = _thisTrigger getVariable '%1'; while{!isNull _thisTrigger && {triggerActivated _thisTrigger}} do {_proxy say3D ('gravi_idle0' + str(floor random 2) ); sleep (5 + (random 20) )}}", QGVAR(idleSound)], ""] // statements
]] call CBA_fnc_globalEventJip;
